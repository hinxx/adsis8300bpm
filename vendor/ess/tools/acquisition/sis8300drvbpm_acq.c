/**
 * BPM based on Struck 8300 Linux userspace acquisition tool.
...
int main(int argc, char **argv) {
    ...
    uint16_t *data[SIS8300DRV_NUM_AI_CHANNELS] = {0};
	...
	status = sis8300drvbpm_init_done(sisuser);
    if (status) {
        printf("sis8300drvbpm_init_done error: %s (%d)\n",
                sis8300drv_strerror(status), status);
        return -1;
    }

	status = sis8300drvbpm_clear_gop(sisuser);
    if (status) {
        printf("sis8300drvbpm_clear_gop error: %s (%d)\n",
                sis8300drv_strerror(status), status);
        return -1;
    }

    status = sis8300drvbpm_clear_pulse_done_count(sisuser);
    if (status) {
        printf("sis8300drvbpm_clear_pulse_done_count error: %s (%d)\n",
                sis8300drv_strerror(status), status);
        return -1;
    }
    ...
    status = sis8300drvbpm_arm_device(sisuser);
    if (status) {
        printf("sis8300drvbpm_arm_device error: %s (%d)\n",
                sis8300drv_strerror(status), status);
        return -1;
    }

    status = sis8300drvbpm_wait_pulse_done_position(sisuser, 2000);
    if (status) {
        printf("sis8300drvbpm_wait_pulse_done_position error: %s (%d)\n",
                sis8300drv_strerror(status), status);
        return -1;
    }
...
*/
	...
	pulseCount = 0;
	status = sis8300drvbpm_get_pulse_done_count(sisuser, &pulseCount);
	 ...
	 * Fix firmware!
	 */
	unsigned int samples[50000] = {0}, iqsamples[50000] = {0}, ctrlreg[50000] = {0};
	...
	for (i = 0; i < 50000; i++) {
		...
		status = sis8300drv_reg_read(sisuser, 0x10, &ctrlreg[i]);
	}
    clock_gettime(CLOCK_REALTIME, &end);
	...
	for (i = 0; i < 50000; i++) {
		printf("[%6i] CTRLREG 0x%04X STAMP %f SAMPLES %10d IQ %10d\n", i, ctrlreg[i], (stamps[i].tv_sec - start.tv_sec) + (stamps[i].tv_nsec - start.tv_nsec) / (double) 1000000000,  samples[i], iqsamples[i]);
	}
	...
	printf("FINAL SAMPLES %10d IQ %10d\n", sampleCount, numIQSamples);
